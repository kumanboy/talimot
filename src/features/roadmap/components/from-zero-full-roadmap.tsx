...
const nodeReasons = {
  phonetics: "Yo‘l xaritangizdagi birinchi tavsiya etilgan mavzu.",
  morphemics: "Fonetika bo‘yicha kamida bir faoliyatni yakunlang.",
  morphology: "Poydevor faoliyatlaridan keyin tavsiya etiladi.",
  syntax: "Morfologiya bo‘yicha talab etilgan faoliyatni yakunlang.",
  stylistics:
    "Grammatika poydevoridan keyin uslubiy bilimlarni mustahkamlang.",
  "scientific-text": "Uslubiyatdan keyingi ilmiy matn tahlili yo‘nalishi.",
  "literary-text": "Uslubiyatdan keyingi badiiy matn tahlili yo‘nalishi.",
  ghazal: "Sintaksis va Grammatika bosqichidan keyin mavjud bo‘ladi.",
  "essay-writing":
...
const stageDescriptions = {
  foundation: "Asosiy tovush va so‘z tuzilishi bilimlarini shakllantiring.",
  grammar: "So‘z turkumlaridan gap qurilishiga ketma-ket o‘ting.",
  "text-and-style":
    "Uslubiyatdan ikki mustaqil matn tahlili yo‘nalishi ochiladi.",
  "literary-analysis": "G‘azal Grammatika bosqichi yakunlangach ochiladi.",
  essay: "Esse yozish uchun ikki zarur yo‘nalish birlashadi.",
  reinforcement:
...
const nodesById = new Map(
  fromZeroRoadmapDefinition.nodes.map((node) => [node.id, node]),
);
...
function getNode(nodeId: RoadmapNodeId) {
  ...
  if (!node) {
    throw new Error(`Roadmap node is missing: ${nodeId}`);
  }
...
function getStage(stageId: RoadmapStageId) {
  const stage = fromZeroRoadmapDefinition.stages.find(
    (candidate) => candidate.id === stageId,
  );

  if (!stage) {
    throw new Error(`Roadmap stage is missing: ${stageId}`);
  }
    ...
    <ol className={styles.nodeSequence}>
      {nodeIds.map((nodeId, index) => (
        <li key={nodeId}>
          <RoadmapNodeView
            nodeId={nodeId}
            onStartPhonetics={onStartPhonetics}
          />
...
function StandardStage({
  ...
  return (
    <section className={styles.stage}>
      ...
      {dependencyLabel ? (
        <p className={styles.dependencyLabel}>{dependencyLabel}</p>
      ) : null}
...
function StageTransition() {
  return (
    <div className={styles.stageTransition} aria-hidden="true">
      <span>↓</span>
    </div>
  );
}
...
export function FromZeroFullRoadmap() {
  ...
  return (
    <RoadmapScreenShell
      ...
      stickyAction={
        <RoadmapStickyAction
          nextStepLabel="Keyingi qadam: Fonetika"
          buttonLabel="Boshlash"
          onAction={startPhonetics}
        />
      }
    >
      <div className={styles.scrollContent}>
        <section className={styles.summary} aria-labelledby="roadmap-summary">
          <h2 id="roadmap-summary">Reja tafsilotlari</h2>
          <dl>
            <div>
              <dt>Maqsad:</dt>
              <dd>A+</dd>
        ...
        <div className={styles.roadmap}>
          <StandardStage
            stageId="foundation"
            onStartPhonetics={startPhonetics}
          />
          <StageTransition />
          ...
          <TextAndStyleStage onStartPhonetics={startPhonetics} />
          <StageTransition />
